...
class MerchantController
{
    ...
    public function __construct($container)
    {
        ...
        $this->method_names = (new GeneralModel())->get_model_methods("MerchantModel");
    }

    public function createMerchant(Request $request, Response $response)
    {
        try {
    ...
    public function getSingleMerchant(Request $request, Response $response, $args)
    {
        $email = $request->getAttribute("email");
        $req_res = [$request, $response];
        $method_identity = $this->method_names;
        if ($method_identity["status"] == true) {
            $resp = (new GeneralModel)->try_get($req_res, $method_identity["message"], 5, $this->output_format, "", $email);
            return $resp;
        } elseif ($method_identity["status"] == false) {
            return $response->withHeader("Content-type", "application/json")
                ->withJson($method_identity)
    ...
    public function getAllMerchant(Request $request, Response $response, $args)
    {
        $req_res = [$request, $response];
        $method_identity = $this->method_names;
        if ($method_identity["status"] == true) {
            $resp = (new GeneralModel)->try_get($req_res, $method_identity["message"], 6, $this->output_format, "", "");
            return $resp;
        } elseif ($method_identity["status"] == false) {
            return $response->withHeader("Content-type", "application/json")
                ->withJson($method_identity)
    ...
    public function verifyEmailAndToken(Request $request, Response $response, $args)
    {
        ...
        $method_identity = $this->method_names;
        if ($method_identity["status"] == true) {
            $resp = (new GeneralModel)->try_get($req_res, $method_identity["message"], 7, $this->output_format, "", $email_token);
            return $resp;
        } elseif ($method_identity["status"] == false) {
            return $response->withHeader("Content-type", "application/json")
                ->withJson($method_identity)
